...
public class SRSHub extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
    ...
    public static class GoBildaPinpoint extends I2CDevice {
        ...
        public static class SetPositionCommand extends Command {
            ...
            public SetPositionCommand(
                    int bus, float xPosition, float yPosition, float hOrientation) {
                data = new BitSet(getLength());
         ...
         */
        public GoBildaPinpoint(float xPodOffset, float yPodOffset, float encoderResolution,
                EncoderDirection xEncoderDirection, EncoderDirection yEncoderDirection) {
        ...
        protected void parseUpdate(BitSet data, int start) {
            ...
            System.arraycopy(
                    deviceStatusChunk, 0, paddedDeviceStatusChunk, 0, deviceStatusChunk.length);
            ...
            System.arraycopy(
                    hOrientationChunk, 0, paddedHOrientationChunk, 0, hOrientationChunk.length);
    ...
    public static class Config {
        ...
        public void setAnalogDigitalDevice(int pin, AnalogDigitalDevice device) {
            if (pin < 1 || pin > 12) {
                throwException(IndexOutOfBoundsException.class,
         ...
         * @throws IllegalStateException if init has already been called or if a device of the same
         *         I2C
                ...
                if (i2cDevice.getAddress() == device.getAddress()) {
                    throwException(IllegalStateException.class,
                            "I2C Bus #" + bus + " "
                                    ...
                                    + "with the " + device.getClass().getName());
                }
            }
    ...
    private void verifyInitialization() {
        if (!isInitialized) {
            byte[] deviceInfo = ByteBuffer
                                        .wrap(deviceClient.read(Register.DEVICE_INFO.address,
            ...
            if (deviceMajorVersion != DEVICE_MAJOR_VERSION
                ...
                throwException(RuntimeException.class,
                        "SRSHub does not report correct firmware version; "
                                + "received v" + deviceMajorVersion + "." + deviceMinorVersion + "."
                                + devicePatchVersion + ", expected v" + DEVICE_MAJOR_VERSION + "."
                                + DEVICE_MINOR_VERSION + "." + DEVICE_PATCH_VERSION);
            }
    ...
    public void init(@NonNull Config config) {
        ...
        int initLength = config.analogDigitalDevices.length * BITS_PER_ANALOG_DIGITAL_DEVICE
        ...
        if (updateLength > 100) {
            throwException(
                    IllegalStateException.class, "Maximum bulk-read length of 100 bytes exceeded");
        }
            ...
            switch (config.analogDigitalDevices[i]) {
                case ANALOG:
                    ...
                    analogDigitalValues[i] =
                            ByteBuffer.wrap(paddedChunk).order(BYTE_ORDER).getShort()
                            / (double) 4095;
        ...
        for (int i = 0; i < config.encoders.length; i++) {
            switch (config.encoders[i]) {
                case QUADRATURE:
                    ...
                    System.arraycopy(
                            quadratureChunk, 0, paddedQuadratureChunk, 0, quadratureChunk.length);

                    encoderValues[i].position = lastPosition
                            + ByteBuffer.wrap(paddedQuadratureChunk).order(BYTE_ORDER).getShort();
                    ...
                    System.arraycopy(quadratureVelocityChunk, 0, paddedQuadratureVelocityChunk, 0,
                            quadratureVelocityChunk.length);

                    encoderValues[i].velocity = ByteBuffer.wrap(paddedQuadratureVelocityChunk)
                                                        .order(BYTE_ORDER)
                                                        .getInt();
                ...
                case PWM:
                    ...
                    encoderValues[i].position =
                            ByteBuffer.wrap(paddedPWMChunk).order(BYTE_ORDER).getShort();
                    ...
                    System.arraycopy(pwmVelocityChunk, 0, paddedPWMVelocityChunk, 0,
     ...
     * @return the current value read from the AnalogDigitalDevice; from 0 to 1 for analog devices
     *         and
    ...
    public double readAnalogDigitalDevice(int pin) {
        ...
        if (config.analogDigitalDevices[pin - 1] == AnalogDigitalDevice.NONE) {
            throwException(IllegalStateException.class,
                    "AnalogDigitalDevice pin #" + pin + " was not configured");
        }
     ...
     *     ticks/ticks per second; for PWM encoders this is in pulse width
     * (microseconds/microseconds per second)
     * @throws IndexOutOfBoundsException if the port is not between 1 and 6, inclusive
     * @throws IllegalStateException if the SRSHub has not yet been initialized
    ...
    public PosVel readEncoder(int port) {
        ...
        if (config.encoders[port - 1] == Encoder.NONE) {
            throwException(
                    IllegalStateException.class, "Encoder port #" + port + " was not configured");
        }
    ...
    public void runCommand(Command command) {
        ...
        System.arraycopy(command.getData().toByteArray(), 0, data, 0,
